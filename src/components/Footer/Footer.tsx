    ...
    <footer className="bg-[#0f313a] text-white py-8 px-4 sm:px-6 md:px-8 lg:px-12 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 h-auto">
      {footerData.map((section, index) => (
        ...
        >
          {section.type === 'logo' ? (
            <>
              <Image width={500} height={500} src={section.logo} alt="Logo" className="mb-4 w-24 sm:w-32" />
              <div className="flex space-x-4 mb-4">
                {section.socialIcons.map((icon, iconIndex) => (
                  <Image width={500} height={500} key={iconIndex} src={icon.src} alt={icon.alt} className="w-5 h-5 sm:w-6 sm:h-6" />
                ))}
              </div>
            ...
            <>
              <div className="flex items-center gap-3 mb-4">
                <Image width={500} height={500} src="/images/arrow.svg" alt="Arrow" className="w-4 h-4 sm:w-5 sm:h-5" />
